...
router.delete("/history", async (req, res) => {
  try {
    ...
    req.log.error({ err }, "Failed to clear translation history");
...
- "contextual_meaning": what this phrase actually means in real conversation (2-3 sentences). Write this in ${targetLanguage}.
- "slang_breakdown": array of objects, each with "term" (keep the original term as-is) and "explanation" (explain it in ${targetLanguage}). Include any slang, idiom, or figurative language detected. If none, return empty array.
- "tone": one of exactly these fixed English values (do not translate): "formal", "casual", "humorous", "sarcastic", "aggressive", "affectionate", "neutral"
- "cultural_notes": any cultural context important for understanding the phrase (1-3 sentences). Write this in ${targetLanguage}.
...
- "detected_language": the language/dialect of the input text (write the language name in ${targetLanguage})
